...
class PenilaianPerilakuController extends Controller
{
    public function index(){
        if(GlobalHelper::cekAkses(Auth::user()->userid,"11")){
            return view('master.penilaian-perilaku.list');
        }else{
    ...
    public function cari(Request $request){
        if(GlobalHelper::cekAkses(Auth::user()->userid,"11")){
            ...
            return view('master.penilaian-perilaku.list',compact('pegawai_dinilai','nama_dinilai','tahun'));
        }else{
            return view('notfound');
        }
    }
